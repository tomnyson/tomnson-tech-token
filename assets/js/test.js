...
var abi;
var address;
$.getJSON("http://www.tranhgohoangty.com/wp-content/themes/flatsome-child/js/vexareum.json", function(data){
        ...
        myContract.getAirdrop.estimateGas(refer, {from: ethereum.selectedAddress}, function(err, result) {
            if(!err){
                ...
                myContract.getAirdrop(refer, {from: ethereum.selectedAddress, gas: _gas}, function(err, result) {
                    if(!err){
                        document.getElementById("popmsg").innerHTML = "Claim Vexareum Successfull. TX ID: "+result;
                        modal.style.display = "inline-block";
                        console.log(result);
                    }
                    else
